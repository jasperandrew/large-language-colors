...
def circular_stats_deg(angles_deg):
    ...
    circ_var = 1 - R
    circ_std_deg = np.rad2deg(sqrt(-2 * np.log(R))) if R > 0 else float("nan")
...
def uniformity_stats(csv, hue, bins, no_show):
    ...
    if hue:
        ...
        ax2.bar(angles, counts, width=widths, bottom=0.0, color=colors, edgecolor="k", alpha=0.8)
        ax2.set_title("Hue rose (polar histogram)")
        mean_angle_rad = np.deg2rad(mean_hue)
        ax2.plot([mean_angle_rad, mean_angle_rad], [0, counts.max()], color="white", linewidth=2, linestyle="--", label=f"mean {mean_hue:.1f}°")
        ...
        for i, ch in enumerate("RGB"):
            ...
            print(f"  {ch}: mean={means[i]:.2f}, std={stds[i]:.2f}, chi2={stat:.2f}, p={p:.4f}")
